...
import { ADD_REFUND, EDIT_REFUND, DELETE_REFUND } from "../actions/refund.action";
        ...
        case ADD_REFUND:
            return state.map((transaction: any) => {
                if (transaction._id === action.payload._id) {
                    return {
                        ...transaction,
                        remboursements: [...transaction.remboursements, action.payload.refund]
                    };
                } else return transaction;
            });
        case EDIT_REFUND:
                ...
                if (transaction._id === action.payload.transactionId) {
                    return {
                        ...
                        remboursements: transaction.remboursements.map((refund: any) =>
                            refund._id === action.payload.refund._id ? action.payload.refund : refund
                        ),
                    };
                } else return transaction;
            });
