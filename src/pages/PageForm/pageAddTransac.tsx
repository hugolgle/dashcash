...
import { getLatestTransactionByTitle, getTitleOfTransactionsByType } from "../../utils/operations";
...
export default function PageAddTransac(props: any) {
  ...
  useEffect(() => {
    if (selectedTitre && lastTransacByTitle) {
      ...
      setSelectedMontant(Math.abs(parseFloat(lastTransacByTitle.montant)).toFixed(2) || '');
    } else {
  ...
  return <>
    <section className="h-full">
      ...
      <form onSubmit={handleSubmit} className='flex flex-col justify-center items-center gap-5 px-36 py-10'>
        ...
        <input value={selectedDate} className="w-96 h-10 px-2 bg-zinc-100 dark:bg-zinc-900 rounded-xl" type="date" onChange={(e) => { handleDateChange(e); handleInputChange(); }} required />

        <textarea value={selectedDetail} className="w-96 h-10 px-2 bg-zinc-100 dark:bg-zinc-900 rounded-xl" placeholder="Détails" maxLength={250} onChange={(e) => { handleDetail(e); handleInputChange(); }} />

        <input value={selectedMontant} className="w-96 h-10 px-2 bg-zinc-100 dark:bg-zinc-900 rounded-xl" type="number" min="0" step="0.01" placeholder="Montant" onChange={(e) => { handleMontant(e); handleInputChange(); }} required />
